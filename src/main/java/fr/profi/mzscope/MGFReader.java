...
public class MGFReader implements Iterator<MSMSSpectrum> {
    ...
    if (value != null) {
      try {
        ...
        if (!result.isEmpty()) {
          ...
          Object[] args = {"retention time", currentLineNumber};
  ...
  private double[] getMassAndIntensity(String value) throws InvalidMGFFormatException {
    ...
    try {
      ...
      Object[] args = {"mass", currentLineNumber};
      String msg = MessageFormat.format("Invalid {0} specified for query [line:{1}]", args);
      throw new InvalidMGFFormatException(msg);
    }
    ...
    if (tokenizer.hasMoreTokens()) {
      ...
      try {
        ...
        Object[] args = {"intensity", currentLineNumber};
        String msg = MessageFormat.format("Invalid {0} specified for query [line:{1}]", args);
        logger.warn(msg);
      }
    }
  ...
  private int getCharge(String value) {
    ...
    try {
      ...
      Object[] args = {"charge", currentLineNumber};
      String msg = MessageFormat.format("Invalid {0} specified for query [line:{1}]", args);
      logger.warn(msg);
    }
    return charge;
